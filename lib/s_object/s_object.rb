module SObject
  ...
  class SObject
    class << self; attr_accessor :sobject_name end

    def initialize(*args)
      # make sure we have a valid object name
      raise RuntimeError.new('SObject does not define an object name') if self.class.sobject_name.blank?
    ...
    def update
      self.class.client.update!(self.class.sobject_name, remote_attributes) ? self : nil
    end

    # Create a new record in salesforce using the values in this record
    def create
      external_id = self.class.client.create!(self.class.sobject_name, remote_attributes)
    ...
    def self.maps_object(name, options={})
      self.sobject_name = "#{name}"

      if options[:custom]
        self.class.sobject_name = "#{self.sobject_name}__c"
      end
    end
  ...
  # Adds a new salesforce field this class will reflect. # @param local [Symbol] The name of the instance variable.
    ...
    # @option options [Symbol] :foreign_key The name of the foreign key instance variable on the related object. Defaults to +:"#{self.class.sobject_name.underscore}_id"+, e.g. +account_id+.
    def self.maps_children(local, options={})
      class_name = options.fetch(:class) { :"#{local.to_s.singularize.capitalize}SObject" }
      local_id = options.fetch(:foreign_key) { :"#{sobject_name.underscore}_id" }
    ...
    def self.find(external_id)
      self.new(client.find(sobject_name, external_id).to_h.symbolize_keys, { translate: true })
    end
    ...
    def self.client
      oauth_token = Rails.cache.read('salesforce_oauth_token')
      force_client = oauth_token.blank? ? Restforce.new : Restforce.new(oauth_token: oauth_token)
      Rails.cache.write('salesforce_oauth_token', force_client.options[:oauth_token])
      force_client
    end
    ...
    def self.where_conditions(args={})
      ...
      args.map do |k,v|
        unless local_fields.keys.include?(k)
          raise ArgumentError.new("'#{k}' is not an attribute of #{sobject_name}")
        end
