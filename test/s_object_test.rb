...
class SObjectTest < Minitest::Test
  ...
  def test_external_id_is_truncated
    assert_equal('123456789012345',
                 TestObject.new(external_id: '123456789012345678').external_id)
    assert_equal('123456789012345',
                 TestObject.new({ Id: '123456789012345678' },
                                translate: true).external_id)
  end

  def test_all_mapped_fields_with_values_in_attributes
    attributes = { external_id: 'somefakeid',
                   my_field1: 'foo',
                   my_field2: 'bar' }
  ...
  def test_field_assignment_on_init_with_strings
    attributes = { external_id: 'somefakeid',
                   my_field1: 'foo',
                   my_field2: 'bar' }
    str_attributes = { 'external_id' => 'somefakeid',
                       'my_field1' => 'foo',
                       'my_field2' => 'bar' }
    obj = TestObject.new
  ...
  def test_usage_of_remote_attributes_when_updating
    ...
    obj.attributes(external_id: 'somefakeid',
                   my_field1: 'myvalue',
                   my_field2: nil)
  end
